use crate::cranelift::debug::{DwarfSectionRelocTarget, ModuleMemoryOffset};
use crate::cranelift::func_environ::FuncEnvironment;
use crate::cranelift::{array_call_signature, native_call_signature, DEBUG_ASSERT_TRAP_CODE};
use crate::cranelift::{builder::LinkOptions, value_type, wasm_call_signature};
...
impl wasmtime_environ::Compiler for Compiler {
    ...
    fn compile_wasm_to_native_trampoline(
        ...
        if let NativeRet::Retptr { offsets, .. } = ret {
            ...
            for (ty, offset) in wasm_func_ty.returns()[1..].iter().zip(offsets) {
                let ty = crate::cranelift::value_type(isa, *ty);
                results.push(
                    builder
                        .ins()
    ...
    fn append_dwarf(
        ...
        let functions_info = funcs
            ...
            .collect();
        let dwarf_sections = crate::cranelift::debug::emit_dwarf(
            &*self.isa,
